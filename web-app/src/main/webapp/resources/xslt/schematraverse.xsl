...
<xsl:param name="currentElementDepth" select="1"/>
<xsl:param name="currentElementPath" select="''"/>
...
<xsl:template match="xsd:element" name="traverseElement" mode="#all">
  <xsl:param name="currentElementDepth" tunnel="yes"/>
  <xsl:param name="currentElementPath" tunnel="yes"/>
  ...
  <xsl:choose>
    ...
    <xsl:when test="@type">
      <xsl:apply-templates select="/" mode="#current">
        ...
        <xsl:with-param name="context" select="."/>
        <xsl:with-param name="currentElementDepth" select="if ($currentElementDepth) then $currentElementDepth+1 else 1" tunnel="yes"/>
        <xsl:with-param name="currentElementPath" select="concat($currentElementPath, '/', @type)" tunnel="yes"/>
  ...
  <xsl:if test="$currentElementDepth and $currentElementDepth &gt;= 20">
      <xsl:message terminate="yes">Maximum allowed schema depth exceeded! <xsl:if test="@type"> (<xsl:value-of select="$currentElementPath"/>)</xsl:if> </xsl:message>
  </xsl:if>
</xsl:template>
