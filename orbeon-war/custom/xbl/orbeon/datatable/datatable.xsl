...
<xsl:transform xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:xhtml="http://www.w3.org/1999/xhtml"
    xmlns:xxforms="http://orbeon.org/oxf/xml/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsd="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:variable name="numberTypes">
        ...
        <type>xsd:nonPositiveInteger</type>
        <type>xsd:negativeInteger</type>
        <type>xsd:long</type>
        <type>xsd:int</type>
        <type>xsd:short</type>
        <type>xsd:byte</type>
    ...
    <xsl:variable name="rowsPerPage"
        select="if ($pass5/fr:datatable/@rowsPerPage castable as xsd:integer) then $pass5/fr:datatable/@rowsPerPage cast as xsd:integer else 10"/>
    <xsl:variable name="maxNbPagesToDisplay"
        select="if ($pass5/fr:datatable/@maxNbPagesToDisplay castable as xsd:integer)
            then if ($pass5/fr:datatable/@maxNbPagesToDisplay cast as xsd:integer mod 2 = 0 )
                then $pass5/fr:datatable/@maxNbPagesToDisplay cast as xsd:integer + 1
                else $pass5/fr:datatable/@maxNbPagesToDisplay cast as xsd:integer
            else -1"/>

    <!-- Variable dealing with sorting and pagination mode -->
    <xsl:variable name="is-externally-sorted-and-paginated" as="xsd:boolean" select="$pass5/fr:datatable/@sortAndPaginationMode = 'external'"/>
    <xsl:variable name="sort-mode" select="$pass5/fr:datatable/@sort-mode"/>
    <xsl:variable name="is-externally-sorted" as="xsd:boolean" select="$sort-mode = 'external' or $is-externally-sorted-and-paginated"/>
    <xsl:variable name="is-internally-sorted" as="xsd:boolean" select="not($is-externally-sorted)"/>
    <xsl:variable name="pagination-mode" select="$pass5/fr:datatable/@pagination-mode"/>
    <xsl:variable name="is-externally-paginated" as="xsd:boolean" select="$pagination-mode = 'external' or $is-externally-sorted-and-paginated"/>
    <xsl:variable name="is-internally-paginated" as="xsd:boolean" select="not($is-externally-paginated)"/>
    ...
    <xsl:function name="fr:significantPositionInRow" as="xsd:integer">
        <xsl:param name="node"/>
    ...
    <xsl:function name="fr:significantElementInRowAtPosition" as="node()">
        <xsl:param name="row" as="node()"/>
        <xsl:param name="index" as="xsd:integer"/>
            ...
            <xforms:model id="datatable-model" xxbl:scope="inner">
                ...
                <!--<xforms:bind nodeset="//column[not(@fr:sortType)]/@type"
                    calculate="for $value in xxforms:evaluate(../@pathToFirstNode)
                        ...
                        else if ($value instance of xsd:decimal)
                            then 'number'
                            else 'text'"/>
-->
                    ...
                    <!--
                       ...
                       <xsl:if test="not($sortAndPaginationMode='external')">
                        ...
                        <xxforms:variable name="nbPages"
                            select="ceiling($nbRows div {$rowsPerPage}) cast as xsd:integer"/>
                        <xforms:bind nodeset="instance('page')">
                            <xforms:bind nodeset="@nbPages" calculate="$nbPages"/>
                            <!-\-<xforms:bind nodeset="."
                               calculate="if (. cast as xsd:integer > $nbPages) then $nbPages else ."
                               />-\->
                        </xforms:bind>
                    </xsl:if>-->
            ...
            <xsl:choose>
                <xsl:when test="$paginated and $is-internally-paginated">
                    ...
                    <xxforms:variable name="nbPages" select="if ($nbRows) then ceiling($nbRows div {$rowsPerPage}) cast as xsd:integer else 0"
                        xxbl:scope="inner"/>
                </xsl:when>
            ...
            <xsl:choose>
                <xsl:when test="$paginated and $maxNbPagesToDisplay &lt; 0">
                    <xxforms:variable name="pages" select="1 to xsd:integer($nbPages)" xxbl:scope="inner"/>
                </xsl:when>
                <xsl:when test="$paginated">
                    <xxforms:variable name="maxNbPagesToDisplay" select="{$maxNbPagesToDisplay} cast as xsd:integer" xxbl:scope="inner"/>
                    <xxforms:variable name="radix" select="floor(($maxNbPagesToDisplay - 2) div 2) cast as xsd:integer" xxbl:scope="inner"/>
                    <xxforms:variable name="minPage"
                        ...
                        else 1) cast as xsd:integer"
                        xxbl:scope="inner"/>
                    <xxforms:variable name="pages"
                        select="if ($nbPages castable as xsd:integer)
                                then 1 to xsd:integer($nbPages)
                                else ()"
                        xxbl:scope="inner"/>
                </xsl:when>
            </xsl:choose>
            ...
            <!-- We don't need pagination links above the table -->
                ...
                <xforms:group model="datatable-model" ref="instance('page')" appearance="xxforms:internal" xxbl:scope="inner">
                    <xforms:input ref="@nbPages" style="display:none;">
                        ...
                        <xforms:setvalue ref=".." ev:event="xforms-value-changed"
                            value="if (. cast as xsd:integer &gt; @nbPages and @nbPages > 0) then @nbPages else ."/>
                    </xforms:input>
                </xforms:group>
            </xsl:if>
                                ...
                                <xsl:choose>
                                    <xsl:when test="$is-externally-sorted">
                                        ...
                                        <xforms:dispatch ev:event="DOMActivate" name="fr-update-sort" target="fr.datatable" xxbl:scope="inner">
                                            <xxforms:context name="fr-column" select="xsd:integer($columnDesc/@index)"/>
                                        </xforms:dispatch>
                                    </xsl:when>
                                    <xsl:otherwise>
            ...
            <xforms:action ev:event="xxforms-nodeset-changed xforms-enabled xforms-disabled" ev:target="#observer">
                ...
                <xsl:if test="$paginated and $is-internally-paginated">
                    ...
                    <!-- <xforms:setvalue model="datatable-model" ref="instance('page')"
                                         value="if (. cast as xsd:integer > $nbPages) then $nbPages else ."/>-->
                </xsl:if>
            </xforms:action>
        ...
        <xsl:if test="$paginated">
            <xhtml:div class="yui-dt-paginator yui-pg-container" style="">
                ...
                <xforms:group appearance="xxforms:internal" xxbl:scope="inner">
                    ...
                    <!-- Riigiportaal ei kasuta First/Last tbalite pagingu juures 
                    ...
                    <xforms:group ref=".[$page = 1]" id="{$prefix}-prev-disabled">
                        	<xhtml:span class="yui-pg-previous">
	                        	<xhtml:strong>
	                        		<!-- <misp-change/> -->
                            		<xhtml:span class="trans_lbl" itemprop="Tagasi|Back|Назад">Tagasi</xhtml:span>
	                        	</xhtml:strong>
                        	</xhtml:span>
                    </xforms:group>
                    <xforms:group ref=".[$page != 1]" id="{$prefix}-enabled">
                        <xforms:trigger class="yui-pg-previous" appearance="minimal" id="{$prefix}-enabled-trigger">
                            <xsl:if test="$is-modal"><xsl:attribute name="xxforms:modal">true</xsl:attribute></xsl:if>
                            	<xforms:label><xhtml:strong>
                            		<xhtml:span class="trans_lbl" itemprop="Tagasi|Back|Назад">Tagasi</xhtml:span>
                            	</xhtml:strong></xforms:label>
                            <xsl:call-template name="fr-goto-page">
                    ...
                    <xhtml:span class="yui-pg-pages">
                        <xforms:repeat nodeset="$pages" id="{$prefix}-pages">
                            ...
                            <xforms:group ref=".[. = $page and $display = 'page']" id="{$prefix}-page-current">
                            	<xhtml:span class="yui-pg-page" id="{$prefix}-page-number">
                            ...
                            <xforms:group ref=".[. != $page and ($display = 'page' or $display = 'ellipses')]" id="{$prefix}-page-goto">
                                <xxforms:variable name="targetPage" select="."/>
                                <xforms:trigger class="yui-pg-page" appearance="minimal" id="{$prefix}-page-goto-trigger">
                                    <xsl:if test="$is-modal"><xsl:attribute name="xxforms:modal">true</xsl:attribute></xsl:if>
                                    <xforms:label>
                                    	<xhtml:span>
                                        	<xforms:output value="if ($display = 'ellipses') then '...' else ."/>
                                        </xhtml:span>
                                    </xforms:label>
                                    <xsl:call-template name="fr-goto-page">
...
<!--                             <xforms:group ref=".[ $display = 'ellipses']" id="{$prefix}-ellipses">
                    ...
                    <xforms:group ref=".[$page = $nbPages or $nbPages = 0]" id="{$prefix}-next-disabled">
                        	<xhtml:span class="yui-pg-next">
	                        	<xhtml:strong>
	                        		<!-- <misp-change/> -->
                            		<xhtml:span class="trans_lbl" itemprop="Edasi|Next|Вперёд">Edasi</xhtml:span>
	                        	</xhtml:strong>
	                        </xhtml:span>
                    </xforms:group>
                    ...
                    <!-- Riigiportaal ei kasuta First/Last tabelite pagingu juures
